...
subroutine cqrdec(m,n,k,Q,ldq,R,ldr,j,rw)
    ...
    !               R -> R1 so that Q1 remains unitary, R1 is upper
    ...
    !               1 <= j <= n.
    ! rw (o)        a real workspace vector of size k-j.
    !
    ...
    ! retriangularize.
    if (j < k) then
        call cqhqr(k+1-j,n-j,R(j,j),ldr,rw,R(1,n))
        ! apply rotations to Q.
        call cqrot('F',m,min(k,n)+1-j,Q(1,j),ldq,rw,R(1,n))
    end if
end subroutine
