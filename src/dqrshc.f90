...
subroutine dqrshc(m,n,k,Q,ldq,R,ldr,i,j,w)
  ...
  use qrupdate_error
    ...
    external dqrot,dcopy,dqrtv1,dqrqh,dqhqr
    ...
    if (i < j) then
        ...
        if (i < k) then
            kk = min(k,j)
            call dqhqr(kk+1-i,n+1-i,R(i,i),ldr,w(k+1),w)
            ! apply rotations to Q.
            call dqrot('F',m,kk+1-i,Q(1,i),ldq,w(k+1),w)
        end if
    else if (j < i) then
        ! shift columns
        call dcopy(k,R(1,i),1,w,1)
        ...
        if (j < k) then
            ...
            call dqrtv1(kk+1-j,R(j,j),w(k+1))
            ! apply rotations to R
            call dqrqh(kk+1-j,n-j,R(j,jj),ldr,w(k+1),R(j+1,j))
            ! apply rotations to Q
            call dqrot('B',m,kk+1-j,Q(1,j),ldq,w(k+1),R(j+1,j))
            ! zero spike.
            do l = j+1,kk
                R(l,j) = 0d0
            end do
